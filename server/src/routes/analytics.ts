...
import { analyzeMessagesForQuestions, groupQuestionsByIntent, groupSimilarQuestions } from '../services/analyticsService';
...
export function setupAnalyticsRoutes(app: Express) {
  ...
  app.get('/api/analytics/questions', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { startDate, endDate } = req.query;

      // Get all customer messages from the specified period
      const messages = await storage.getCustomerMessages(userId, startDate as string, endDate as string);
      ...
      const groupedQuestions = await groupQuestionsByIntent(businessQuestions);
  ...
  app.post('/api/analytics/generate-faq', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { startDate, endDate, minCount = 2 } = req.body;

      // Get customer messages and analyze questions
      const messages = await storage.getCustomerMessages(userId, startDate, endDate);
      const businessQuestions = await analyzeMessagesForQuestions(messages, userId);
